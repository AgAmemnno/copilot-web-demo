...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      ...
      'process.env.GH_PAT_FOR_TEST':JSON.stringify(env.GH_PAT_FOR_TEST),
      'process.env.TEST_GITHUB_OWNER': JSON.stringify(env.TEST_GITHUB_OWNER),
      'process.env.TEST_GITHUB_REPO': JSON.stringify(env.TEST_GITHUB_REPO),
      'process.env.TEST_GITHUB_BRANCH': JSON.stringify(env.TEST_GITHUB_BRANCH)
    ...
    test: {
      ...
      include: [
        'src/**/githubApiAnalyze.test.{js,ts}', // src ディレクトリ配下の .test.js と .test.ts ファイル
