...
describe('GitHub GraphQL API ファイル一覧＋サイズ取得テスト', () => {
  ...
  it('GraphQLで指定ブランチのファイル一覧とサイズを取得できる', async () => {
    const files = await fetchRepoFileTreeWithSize(owner, repo, branch);
    expect(files).toBeDefined();
    expect(Array.isArray(files)).toBe(true);
    expect(files.length).toBeGreaterThan(0);

    // 各ファイルに size 属性があることを確認
